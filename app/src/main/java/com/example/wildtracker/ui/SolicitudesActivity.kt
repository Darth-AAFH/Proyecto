...
class SolicitudesActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private fun CargarSolicitidesEnviadas() {
        ...
        var mListView =
            findViewById<ListView>(R.id.solicitudesList) //Seteamos la lista de usuarios en una variable para poder manipularla
        ...
        db.collection("users").document(MainActivity.user!!).collection("SolicitudesAmistad").get()
    ...
    private fun CargarSolicitidesRecibidas() {
        ...
        values.clear() // Limpiamos la vista cada que se presione el boton para evitar escribir mas de alguna vez el dato
        var mListView =
            findViewById<ListView>(R.id.solicitudesListRecibidas) //Seteamos la lista de usuarios en una variable para poder manipularla
        mListView.emptyView //Vaciamos la lista
        mListView.adapter = ArrayAdapter(
            this,
            android.R.layout.simple_list_item_1, values
    ...
    private fun AlertaSolicitudEnviada(perfil: String) {
        ...
        builder.setTitle("Enviaste una solicitud a ${perfil}")
            ...
            .setNeutralButton("Eliminar") { dialogInterface, it ->
                ...
                db.collection("users").document(MainActivity.user!!)
                    .collection("SolicitudesAmistad").get().addOnSuccessListener { result ->
                    //Consulta en la base de datos los usuarios que coicidan con el nombre de usuario a dejar de seguir, cuando lo encuentra lo elimina
                    for (document in result) {
                ...
                db.collection("users").document(MainActivity.user!!)
                    ...
                    for (document in result) {
                        ...
                        if (perfilGet == perfil) {
                            ...
                            db.collection("users").document(MainActivity.user!!)
                                .collection("SolicitudesAmistad").document(document.id)
...
//Toast.makeText(this,perfil,Toast.LENGTH_SHORT).show()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arrayAdapterRecibidas = ArrayAdapter(
            this,
        ...
        mListView!!.setOnItemClickListener { parent, view, position, id ->
            ...
            var Perfil: String =
                MainActivity.listaSolicitudesEnviadas[(MainActivity.listaSolicitudesEnviadas.size.toInt() - position.toInt()) - 1]
        ...
        mListViewRecibidas!!.setOnItemClickListener { parent, view, position, id ->
            ...
            var Perfil: String =
                MainActivity.listaSolicitudesRecibidas[(MainActivity.listaSolicitudesRecibidas.size.toInt() - position.toInt()) - 1]
